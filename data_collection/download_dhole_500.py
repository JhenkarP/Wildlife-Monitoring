"""Download Dhole photos observed in India from iNaturalist."""
...
TAXON_ID = 42101
INDIA_PLACE_ID = 6681
OUTPUT_ROOT = ROOT / "data" / "training" / "images" / "dhole"
...
def main() -> None:
    ...
    session.headers["User-Agent"] = "Animal-Monitoring Dhole India dataset collector"
        ...
        for observation in observations:
            for photo in observation.get("photos") or []:
                ...
                try:
                    ...
                    manifest_rows.append({"image_path": image_path.relative_to(OUTPUT_ROOT).as_posix(), "label": "dhole", "observation_id": observation["id"], "photo_id": photo_id, "taxon": observation.get("taxon", {}).get("name", "Cuon alpinus"), "place_guess": observation.get("place_guess", ""), "source_url": f"https://www.inaturalist.org/observations/{observation['id']}", "photo_url": photo_url, "license": photo.get("license_code") or "unknown", "attribution": photo.get("attribution", "")})
