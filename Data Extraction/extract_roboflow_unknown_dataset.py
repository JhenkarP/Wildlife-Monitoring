"""Extract unknown-label images from a Roboflow COCO export."""
...
IMAGE_ROOT = ROOT / "data" / "training" / "images" / "unknown"
ZIP_PATH = Path.home() / "Downloads" / "gaur.v1-gaur-review-export.coco.zip"
EXPECTED_IMAGES = 285


def collect_unknown_images(archive: zipfile.ZipFile) -> list[tuple[str, str]]:
    ...
    for annotation_name in sorted(archive.namelist()):
        ...
        selected.extend(
            ...
            if image_labels[image["id"]] in ({"1"}, set())
        )
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="roboflow-unknown-") as temporary_directory:
        staging_root = Path(temporary_directory) / "unknown"
        staging_root.mkdir()
        with zipfile.ZipFile(ZIP_PATH) as archive:
            selected_images = collect_unknown_images(archive)
            if len(selected_images) != EXPECTED_IMAGES:
                raise SystemExit(
                    f"Expected {EXPECTED_IMAGES} unknown images, found {len(selected_images)}"
                )
            for index, (split, source_name) in enumerate(selected_images, start=1):
                suffix = Path(source_name).suffix.lower() or ".jpg"
                destination = staging_root / f"unknown_{index:04d}{suffix}"
